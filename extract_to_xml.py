...
def parse_commandline(args: List[str]) -> Namespace:
    def database(path: str):
        if not Path(path).is_file() or not Path(path).suffix == ".sqlite":
            raise ArgumentTypeError(f"{path} not a valid Mixxx database file")
        return path

    parser = ArgumentParser(description="Create Rekordbox XML file from Mixxx database")
    parser.add_argument(
        "input", type=database, help="Path to Mixxx SQLite database file"
    )
...
def main() -> None:
    ...
    root_node = ET.SubElement(
        ...
        Name="ROOT",
        Type="0",
        Count=str(len(playlist_tracks_dict)),
    )
